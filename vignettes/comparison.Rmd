...
traps_cols <- c("#252424", RColorBrewer::brewer.pal(8, "Set2")[c(7, 4, 6, 2, 3, 5, 1)])
...
The relative differences from the Hirst mean are substantial, Hund, Wibs and Poleno measure more pollen than Hirst; KHB and Rapide measure less. 
...
map2(errors, aggregation, function(first, second) {
  gg_errors1 <- first %>%
    ...
    geom_boxplot() +
    labs(y = "Relative Difference from Hirst (Log)", x = "") +
    scale_fill_manual(values = traps_cols) +
    ggtitle(paste0(
      "Boxplot of ",
      tools::toTitleCase(second),
