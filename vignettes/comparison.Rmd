...
For higher temporal resolution rapide, KH and Hirst show some peaks, that might be a data issue (especially true for rapide).
Most traps were able to identify days with higher pollen occurence, but the performance varies a lot (maybe also species dependent).
...
map2(pollen, aggregation, function(first, second) {
  ...
  gg_time12 <- ggarrange(gg_time1, gg_time2, ncol = 1)
...
Looking at the boxplots we can see how the spread in the measurements increses with higher temporal resolution; especially for Hirst.
...
Looking at the histogram, the findings above are solidified.
Furthermore we clearly see the discrete timeseries produced by Hirst Traps and Manual Counting for higher resolutions.
...
Looking at relative differences between the traps (i.e. from the common mean). Gives a clearer picture of the sources of uncertainties.
...
errors <- map(pollen, ~ .x %>%
  ...
  map(~ .x %>%
    pivot_longer(hirst1:wibs, names_to = "trap", values_to = "conc") %>%
    mutate(reldiff = conc / mean,
...
The relative differences from the common mean are larger than 100% for the Hirst and Poleno traps.
The spread in errors is very large for those traps as well.
This does not mean that they measure worse as the rest. There is no golden standard in pollen Measuring.
KHB and Rapide consistently measuring less, lowers the common mean.
...
map2(errors, aggregation, function(first, second) {
  gg_errors1 <- first %>%
    ...
    geom_boxplot() +
    labs(y = "Relative Difference from Common Mean", x = "") +
    scale_fill_manual(values = traps_cols) +
    ggtitle(paste0(
      "Boxplot of ",
      tools::toTitleCase(second),
...
For lower concentrations Poleno is measuring more then the rest (better capture?).
KHB and Rapide are consitently measuring less pollen than the other traps in the study.
```{r concerrors}
...
The table shows the relative differences for all traps combined.
...
kable_error <- map(errors_conc, ~.x %>%
  ...
  pack_rows("Six-Hour", 6, 12, indent = FALSE) %>%
  pack_rows("Hourly", 13, 19, indent = FALSE) %>%
  add_header_above(c("Relative Differences for All Traps Combined" = 5))
...
The density plots outline the behavior of all traps for different concentration categories (based on the common mean).
KHB and Rapide stant out, usually measuring less than the others for concentrations below 300.
Poleno further stand out with more pollen measured during periods of low concentrations, and a sharp spike in the 300 conc_group.

```{r concerror_kable, eval = FALSE}
map2(errors_conc, aggregation, function(data, second) {
  gg_conc_dens <- map(conc_groups, function(first) {
    conc_plot_data <- data %>%
      filter(group == first)
  ...
  gg_density1 <- ggarrange(
    plotlist = Filter(Negate(is.null), gg_conc_dens),
    ncol = 2,
    nrow = length(Filter(
      Negate(is.null),
      gg_conc_dens
...
Pollendata usually does not fulfil the assumption of ANOVA. We want to check this in the following.
...
```{r log}
pollen_log <- map(pollen, ~ .x %>% mutate(conc = log(conc + 1)))
```

```{r QQ}
map2(pollen, pollen_log, function(first, second) {
...
It allows us to check whether the residuals have constant variance and whether the residuals have mean zero (i.e. they don’t show any deterministic pattern). 
...
map2(pollen, pollen_log, function(first, second) {
  ...
  resid_df_log <- tibble(resid = resid, id = as.numeric(names(resid_log)))
  ...
  ggarrange(gg_timeline1, gg_timeline2, nrow = 2) %>%
    annotate_figure(top = paste("Index-Plot for the ANOVA Residuals",
                                 "With (left) and Without Logarithmizing"))
})
```
...
We are using the robust method from Kendall, comparing Tau. 
It measures the number of concordant - discordant pairs in two ranked sets of observations.
(https://en.wikipedia.org/wiki/Kendall_rank_correlation_coefficient)

```{r corrdata}

data_corr <- map(pollen, ~ .x %>%
...
corr_tb <- map(corr_matrix, ~ .x %>%
  ...
  map_at(c(2, 3), ~.x %>%
  mutate(hund = NA_real_,
         wibs = NA_real_)) %>%
  map(~ .x %>% select(trap, traps_names))
...
kable_corr <-  corr_tb %>%
  ...
  pack_rows("Six-Hour", 9, 14, indent = FALSE) %>%
  pack_rows("Hourly", 15, 20, indent = FALSE) %>%
  add_header_above(c("Kendall Tau for Different Temporal Resolutions" = 9))
...
The test is generally considered to be robust to ties. However, if ties are present they should not be concentrated together in one part of the distribution 
(they should have either a normal or uniform distribution)
https://influentialpoints.com/Training/kruskal-wallis_anova-principles-properties-assumptions.htm

The Wilcoxon signed-rank test assumes that the data are distributed symmetrically around the median. In other words, there should be roughly the same number of 
...
```{r nparcomp}
kable_nparcomp <- map2(pollen_log, aggregation, function(first, second) {
