...
The data has missing values in the Poleno and RapidE timeseries. For Poleno calibration was carried out on 6 days in May.
For RapidE software mulfunctioned (it seems) during 7 days, as discussed with MDS.
These full 13 days were excluded from the analysis for all traps and temporal resolutions.
The whole analysis is based on the data exlcuding these 13 days, except for the timeseries plot (figure 1).
For the timeseries the full timeseries are used, which includes NAs for Poleno and Rapid-E.
...
kable_ntraps <- pmap(
  list(number_of_measurements, aggregation, c(7, 7, 5)),
...
traps_cols <- RColorBrewer::brewer.pal(7, "Set2")[c(1, 7, 4, 6, 2, 3, 5)]
...
Whereas the mean was calculated across the full period.
...
Looking at relative differences between Hirst and the other traps, 
...
It is common to exclude measurements above a 10 pollengrains per m^3 threshold.
Hirst often measures 0 pollen grains, as the measured concentrations 
by this device are of discrete nature for low concentrations.
...
map2(errors_conc, aggregation, function(first, second) {
  ...
  filter(trap != "hirst") %>%
    ...
    geom_boxplot() +
    labs(y = "Relative Difference from Hirst Mean", x = "") +
    scale_fill_manual(values = traps_cols) +
    ggtitle(paste0(
      "Boxplot of ",
      tools::toTitleCase(second),
...
kable_corr <-  corr_tb %>%
  ...
  pack_rows("Daily", 1, 7, indent = FALSE) %>%
  pack_rows("Six-Hour", 8, 14, indent = FALSE) %>%
  pack_rows("Hourly", 15, 19, indent = FALSE) %>%
  add_header_above(c("Spearman Rho for Different Temporal Resolutions" = 8))
