...
The data has no missing values except for the Poleno timeseries, for which calibration was carried out on 6 days in March.
...
For higher temporal resolution rapide, KH and Hirst show some peaks, 
that might be a data issue (especially true for rapide).
...
Looking at relative differences between the traps (i.e. from the common mean), 
one can investigate which traps diverge the most from the common mean.
...
The spread of relative differences from the common mean is very large for those traps as well.
...
The table shows the relative differences from the common mean for all traps combined.
...
and a sharp spike in the 300 conc_group.
